...
```prisma
@@index([title])       -- supports ILIKE search on event title
...
Without the `eventDate` index, sorting + paginating 100,000+ rows would require a full table scan on every request. With it, PostgreSQL can use an index scan for the filter and sort.

**Known limitation:** `ILIKE '%term%'` with a leading wildcard cannot use a standard B-tree index — it falls back to a sequential scan on the title column. The proper fix is a `pg_trgm` GIN index (`CREATE INDEX ON "Event" USING GIN (title gin_trgm_ops)`), which supports trigram-based fuzzy matching and works with leading wildcards. This was omitted to keep the migration simple, but would be the first performance improvement at scale.

A composite index `(title, eventDate)` would be slightly more efficient for combined queries but adds write overhead and would not help queries that filter on `eventDate` alone.
